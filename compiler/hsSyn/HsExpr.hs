...
data StmtLR idL idR body -- body should always be (LHs**** idR)
  ...
  -- | 'ApplicativeLastStmt' represents an applicative expression built with
  ...
  -- > join ((\x1...xn -> e) <$> e1 <*> ... <*> en)
  --
  -- The form of the applicative expression is generalised so that instead
  -- of collecting a set of single BindStmts, we collect a series of
  -- groups of BindStmts.  The body may also be a sequence of stmts instead
  -- of a simple expression.  More details in Note [ApplicativeDo].
  --
  | ApplicativeLastStmt
             (ApplicativeBody idL)
             [ ( ApplicativeArg idL
...
data ApplicativeArg id
 = ApplicativeArgOne (LStmtLR id id (LHsExpr id))
 | ApplicativeArgMany
     (LPat id)
     [LStmtLR id id (LHsExpr id)]
     (PostTc id Type)
  deriving Typeable

deriving instance DataId id => Data (ApplicativeArg id)

data ApplicativeBody id
 = ApplicativeBodyExpr (LHsExpr id)
 | ApplicativeBodyStmts [ExprLStmt id]
 deriving Typeable

deriving instance DataId id => Data (ApplicativeBody id)
...
pprStmt (ApplicativeLastStmt body pairs mb_join _)
  ...
  -- flattening the whole ApplicativeLastStmt nest back to a sequence
  -- of statements.
   pp_for_user =
      vcat $ map (<> semi) $
         (concatMap flattenArg (map fst pairs) ++ flattenBody body mb_join)

   -- ppr directly rather than transforming here, becuase we need to
   -- inject a "return" which is hard when we're polymorphic in the id
   -- type.
   flattenStmt :: OutputableBndr id => ExprLStmt id -> [SDoc]
   flattenStmt (L _ (ApplicativeLastStmt body pairs mb_join _)) =
     concatMap flattenArg (map fst pairs) ++ flattenBody body mb_join
   flattenStmt stmt = [ppr stmt]

   flattenArg :: OutputableBndr id => ApplicativeArg id -> [SDoc]
   flattenArg (ApplicativeArgOne stmt) = flattenStmt stmt
   flattenArg (ApplicativeArgMany _ stmts _) =
     init (concatMap flattenStmt stmts)
     -- the final stmt in an ApplicativeArgMany is always the return
     -- we generated in the renamer, so elide it when printing for the user.

   flattenBody :: OutputableBndr id => ApplicativeBody id -> Maybe a -> [SDoc]
   flattenBody (ApplicativeBodyExpr e) mb_join
     | isNothing mb_join = [ptext (sLit "return") <+> ppr e]
     | otherwise = [ppr e]
   flattenBody (ApplicativeBodyStmts stmts) _ =
     concatMap flattenStmt stmts

   pp_debug =
     let
         fun = char '\\' <> hsep (map ppr pats) <+> arrow <+> pp_body
   ...
   (pats, exprs) =
      unzip [ case arg of
                ApplicativeArgOne (L _ (BindStmt p e _ _)) -> (p,e)
                ApplicativeArgMany p stmts _ ->
                  (p, noLoc (HsDo DoExpr stmts (error "pprStmt")))
            | (arg, _) <- pairs ]

   pp_body =
     case body of
       ApplicativeBodyExpr body
         | isNothing mb_join -> ptext (sLit "return") <+> ppr body
         | otherwise -> ppr body
       ApplicativeBodyStmts stmts ->
         ppr (HsDo DoExpr stmts (error "pprBodyDebug"))

instance OutputableBndr id => Outputable (ApplicativeArg id) where
    ppr arg = pprApplicativeArg arg

pprApplicativeArg :: OutputableBndr id => ApplicativeArg id -> SDoc
pprApplicativeArg (ApplicativeArgOne stmt) = ppr stmt
pprApplicativeArg (ApplicativeArgMany pat stmts _) =
  ppr pat <+> larrow <+> ppr (HsDo DoExpr stmts (error "pprApplicativeArg"))
