...
-- putting any error messages under the herald provided.
checkListBy :: (a -> a -> Maybe SDoc) -> [a] -> [a] -> SDoc
            -> Maybe SDoc
checkListBy check_fun as bs herald = go [] as bs
  where
    go []   [] [] = Nothing
    ...
    go _    _  _ = Just (hang (herald <> colon)
                            2 (text "There are different numbers of elements"))
...
checkBootTyCon tc1 tc2
  ...
  = let
    ...
    checkListBy eqSig op_stuff1 op_stuff2
                (text "The methods do not match") `andThenCheck`
    checkListBy eqAT ats1 ats2
                (text "The associated types do not match")
  ...
  where
    ...
    eqAlgRhs _  tc1@DataTyCon{} tc2@DataTyCon{} =
        checkListBy eqCon (data_cons tc1) (data_cons tc2)
                    (text "The constructors do not match")
